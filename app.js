...
function enterScreen(next,reason="render",trigger=pendingTransitionCause.trigger||"internal"){
 ...
 runtimeSessionId++;renderGeneration++;state.screen=next;const localBuildProof=document.getElementById("losLocalBuildProof");if(localBuildProof)localBuildProof.hidden=next==="home";
...
function createHostSystem(provider=defaultHostProvider()){
 let token=0,eventSequence=0,speaking=false,currentPriority=0,hostGateActive=false,lastIds=[],lastFamilies=[],idleWaiters=[],currentEntry=null;
 const history=[];
 ...
 const cancel=(reason,releaseHostGate=true)=>{token++;provider.cancel?.(reason);if(currentEntry&&!/completed|failed|cancelled/.test(currentEntry.result||"")){currentEntry.result="cancelled";currentEntry.cancelReason=reason;currentEntry.settledAt=Date.now()}const outcome=currentEntry;currentEntry=null;const release=hostGateActive;hostGateActive=false;speaking=false;currentPriority=0;if(release&&releaseHostGate)beginPostHostQuarantine(outcome);settleIdle(outcome);return release};
 ...
 const emit=(event,context={})=>{
  ...
  const carriedGate=speaking?cancel("interrupted-by-"+event,false):false;
  const myToken=++token;currentPriority=priority;speaking=true;hostGateActive=carriedGate||temporarilySuppressTranscriptsForHost(history.at(-1));
  ...
  Promise.resolve(provider.play({hostEventId:historyEntry.hostEventId,event,text:line.text,priority:level,volume:state.volume,context:{...context,screen:state.screen,runtimeSession:runtimeSessionId,renderGeneration}})).then(()=>{historyEntry.result="playback-completed";historyEntry.playbackCompletedAt=Date.now()}).catch(error=>{historyEntry.result="playback-failed";historyEntry.error=String(error?.message||error)}).finally(()=>{
   if(myToken!==token)return;speaking=false;currentPriority=0;historyEntry.settledAt=Date.now();currentEntry=null;const release=hostGateActive;hostGateActive=false;if(release)beginPostHostQuarantine(historyEntry);settleIdle(historyEntry)
  });return true
 };
...
function clearRuntime(){clearPendingPlayersNavigation();clearPendingAnswerCandidate();voiceCore?.interimCandidates?.clear();hostSystem?.cancel("runtime-change");questionReading=false;answerListening=false;if(voiceCore){clearTimeout(voiceCore.quarantineTimer);voiceCore.quarantineTimer=null;setAnswerGate("turn-ended")}audioDiagnostics.tick="off";GameAudio.restore();GameAudio.stopPending();clearInterval(questionTimer);questionTimer=null;clearTimeout(answerGraceTimer);answerGraceTimer=null;clearTimeout(flowTimer);flowTimer=null;questionSoundTimers.forEach(clearTimeout);questionSoundTimers=[];handoffTimers.forEach(clearTimeout);handoffTimers=[];celebrationTimers.forEach(id=>{clearTimeout(id);clearInterval(id)});celebrationTimers=[]}
...
function updateRetryMicAvailability(){
 ...
 const reason=voiceCore.permissionBlocked?"permission":hostSystem?.isSpeaking()||voiceCore.answerGate==="host-suppressed"?"host":voiceCore.answerGate==="post-host-quarantine"?"quarantine":"";
 button.disabled=!!reason;button.setAttribute("aria-disabled",String(!!reason));button.textContent=reason==="permission"?"MIC PERMISSION NEEDED":reason==="host"?"HOST SPEAKING":reason==="quarantine"?"MIC READYING":"TRY MIC AGAIN"
}
const POST_HOST_QUARANTINE_MS=200;
function clearVoiceTranscriptResidue(reason){clearPendingPlayersNavigation();clearPendingAnswerCandidate();voiceCore.handledInterimSlots.clear();voiceCore.handledFinalSlots.clear();voiceCore.interimCandidates.clear();voiceCore.currentUtterance=null;voiceCore.answerUtterance=null;voiceDiagnostic("voice-transcript-residue-cleared",{reason,questionSessionId})}
function setAnswerGate(gate,stage="answer-gate-changed",detail={}){voiceCore.answerGate=gate;voiceCore.answerGateChangedAt=performance.now();voiceDiagnostic(stage,{answerGate:gate,recognitionState:voiceCore.actualState,recognitionGeneration:voiceCore.generation,restartCount:voiceCore.restartCount,questionSessionId,activePlayer:state.game?.players?.[state.game?.idx]?.name||null,...detail});return gate}
function normalAnswerGate(){if(state.screen==="paused")return"paused";if(state.screen!=="question")return"disabled";if(!answerListening||state.game?.answered)return"turn-ended";return"active"}
function temporarilySuppressTranscriptsForHost(entry){
 if(!state.voiceOn)return false;clearTimeout(voiceCore.quarantineTimer);voiceCore.quarantineTimer=null;clearVoiceTranscriptResidue("host-suppression-start");voiceCore.hostSuppressionStartedAt=performance.now();voiceCore.suppressionReason="host-speech";setAnswerGate("host-suppressed","host-suppression-start",{hostEventId:entry?.hostEventId||null});updateRetryMicAvailability();if(!recognition&&voiceCore.desired&&!voiceCore.permissionBlocked)startVoice(state.screen);return true
}
function beginPostHostQuarantine(entry){
 clearTimeout(voiceCore.quarantineTimer);clearVoiceTranscriptResidue("host-playback-end");voiceCore.hostPlaybackEndedAt=performance.now();voiceDiagnostic("host-playback-end",{hostEventId:entry?.hostEventId||null,questionSessionId,recognitionState:voiceCore.actualState,recognitionGeneration:voiceCore.generation,restartCount:voiceCore.restartCount});voiceCore.suppressionReason="post-host-quarantine";setAnswerGate("post-host-quarantine","post-host-quarantine-start",{durationMs:POST_HOST_QUARANTINE_MS,hostEventId:entry?.hostEventId||null});
 voiceCore.quarantineTimer=setTimeout(()=>{voiceCore.quarantineTimer=null;clearVoiceTranscriptResidue("post-host-quarantine-end");voiceCore.suppressionReason="";const gate=normalAnswerGate();setAnswerGate(gate,"post-host-quarantine-end",{durationMs:POST_HOST_QUARANTINE_MS,hostEndToGateActiveMs:Math.max(POST_HOST_QUARANTINE_MS,Math.round(performance.now()-voiceCore.hostPlaybackEndedAt))});updateRetryMicAvailability();if(!recognition&&state.voiceOn&&voiceCore.desired&&!voiceCore.permissionBlocked)startVoice(state.screen)},POST_HOST_QUARANTINE_MS)
}
...
let voiceCore={ctx:null,owner:null,restart:null,errorWatchdog:null,quarantineTimer:null,generation:0,retryAttempt:0,restartCount:0,desired:false,lastDesiredDecision:{desired:false,reason:"initializing"},actualState:"stopped",answerGate:"disabled",answerGateChangedAt:null,hostSuppressionStartedAt:null,hostPlaybackEndedAt:null,firstAcceptedAfterHostQuestion:null,health:{generation:0,phase:"stopped",events:{}},lastManualRecoveryAt:-Infinity,lastKey:"",lastAt:0,lastHeard:"",lastTranscript:"",lastTranscriptFinal:null,lastInterim:"",lastFinal:"",lastRoute:"",lastRejection:"",lastError:"",suppressionReason:"",startRequestedAt:null,startedAt:null,listeningAt:null,lastSpeechAt:null,endedAt:null,permissionBlocked:false,handledInterimSlots:new Map(),handledFinalSlots:new Map(),interimCandidates:new Map(),pendingAnswerCandidate:null,pendingAnswerTimer:null,navQueued:null,pendingPlayersNav:null,pendingPlayersNavTimer:null,diagnostics:[],utterance:0,currentUtterance:null,answerUtterance:null,latencySeen:new Set()},lastPlayerVoiceMutation={key:"",at:0},voiceLifecycleSuspended=voicePageHidden();
...
function acceptWestCoastVoiceAnswer(h,confidence,isFinal,match){
 ...
 return voiceOnce("answer:"+questionSessionId+":"+norm(g.current?.a||""),()=>{if(g.answerAcceptedAt==null){g.answerAcceptedAt=performance.now();g.answerAcceptedTranscript=String(h||"");g.answerAcceptedMatch=match.method;g.answerAcceptedPlayerId=g.players[g.idx]?.id||null;if(voiceCore.hostPlaybackEndedAt!=null&&voiceCore.firstAcceptedAfterHostQuestion!==questionSessionId){voiceCore.firstAcceptedAfterHostQuestion=questionSessionId;voiceDiagnostic("first-player-transcript-after-host",{questionSessionId,transcript:h,gateActiveToTranscriptMs:Math.max(0,Math.round(g.answerAcceptedAt-voiceCore.answerGateChangedAt)),hostEndToTranscriptMs:Math.max(0,Math.round(g.answerAcceptedAt-voiceCore.hostPlaybackEndedAt)),recognitionGeneration:voiceCore.generation,restartCount:voiceCore.restartCount})}voiceDiagnostic("answer-accepted",{questionSessionId,acceptedAt:g.answerAcceptedAt,playerId:g.answerAcceptedPlayerId,transcript:h,matchMethod:match.method,matchedAnswer:match.matched||null,isFinal:!!isFinal})}recordAnswerAttempt(h,true,confidence,isFinal,match);voiceFeedback("✓ ANSWER","action");finish("correct")})
}
...
function startVoice(ctx){
 ...
 try{
  ...
  r.onresult=e=>{
   ...
   if(voiceCore.answerGate==="host-suppressed"||voiceCore.answerGate==="post-host-quarantine"){
    for(let i=e.resultIndex;i<e.results.length;i++){const text=(e.results[i]?.[0]?.transcript||"").trim();if(text)voiceDiagnostic("host-transcript-ignored",{text,isFinal:!!e.results[i]?.isFinal,resultIndex:i,reason:voiceCore.answerGate,questionSessionId,recognitionGeneration:generation})}
    clearVoiceTranscriptResidue(voiceCore.answerGate);return
   }
...
function question(resumeCurrent=false){
 ...
 const input=document.getElementById("typedAnswer"),lock=document.getElementById("lockAnswer"),retryMic=document.getElementById("retryMic");if(retryMic)retryMic.onclick=manualRecoverVoice;answerListening=true;setAnswerGate("active","answer-gate-active",{reason:resuming?"question-resumed":"question-ready"});startVoice("question");
...
function westCoastQuestionIdentityMarkup(player,label="Current player"){
 const name=String(player?.name||"PLAYER"),nameSize=name.length>24?"name-long":name.length>12?"name-medium":"name-regular";
...
function result(outcome,resumeDelay=null,revealAnswer=false){
 ...
 if(outcome==="correct"){
  const playerName=String(p?.name||"PLAYER"),playerNameSize=playerName.length>24?"name-long":playerName.length>12?"name-medium":"name-regular";
...
function champion(p){
 ...
 const mobileLandscapeWinner=globalThis.matchMedia?.("(orientation:landscape) and (max-width:950px) and (max-height:450px)")?.matches===true,winnerNameMinPx=mobileLandscapeWinner?14:28;
 fitFocalText(document.querySelector(".champion-name"),{container:document.querySelector(".champion-box"),minPx:winnerNameMinPx,multiline:true});if(typeof requestAnimationFrame==="function")requestAnimationFrame(()=>{if(state.screen==="complete")fitFocalText(document.querySelector(".champion-name"),{container:document.querySelector(".champion-box"),minPx:winnerNameMinPx,multiline:true})});
...
async function installLocalBuildProof(){
 ...
 try{
  ...
  badge.hidden=state.screen==="home";document.body.appendChild(badge);globalThis.__LOS_LOCAL_BUILD_PROOF__=proof
