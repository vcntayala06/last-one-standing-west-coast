...
function westCoastGlobalControlsMarkup({showMic=true}={}){return `<nav class="wc-global-controls" aria-label="Screen controls"><button type="button" id="back" class="wc-global-back"><span aria-hidden="true">←</span><strong>BACK</strong></button><div class="wc-global-tools">${showMic?westCoastMicMarkup():""}${westCoastSettingsMarkup()}</div></nav>`}
...
function savedProfiles(){try{const saved=JSON.parse(localStorage.getItem(STORAGE.profiles)||"[]");if(Array.isArray(saved)&&saved.length)return saved.map(normalizePlayer);const legacy=remembered().map(name=>normalizePlayer({id:uid(),firstName:name,name,hostStyle:"neutral"}));if(legacy.length)localStorage.setItem(STORAGE.profiles,JSON.stringify(legacy));return legacy}catch{return[]}}
...
const HOME_ART_COORDS={
 landscape:{width:1672,height:941,play:[598,720,524,126],how:[1340,28,229,64],settings:[1580,27,66,66]},
 portrait:{width:941,height:1672,play:[158,1335,627,191],how:[527,36,291,79],settings:[829,36,79,79]},
 ultrawide:{width:1844,height:853,play:[600,674,629,150],how:[1405,36,300,79],settings:[1722,36,82,82]}
};
function positionHomeHotspots(){
 const plate=document.querySelector(".los-home-final .los-home-plate");if(!plate)return;
 const orientation=innerWidth<=innerHeight?"portrait":innerWidth/innerHeight>=2?"ultrawide":"landscape",source=HOME_ART_COORDS[orientation],scale=Math.max(innerWidth/source.width,innerHeight/source.height),cropX=(source.width*scale-innerWidth)*(orientation==="ultrawide"?.5:.85),cropY=(source.height*scale-innerHeight)/2;
 for(const [name,box] of Object.entries({play:source.play,how:source.how,settings:source.settings})){
...
function home(){
 ...
 app.innerHTML=`<section class="screen los-home-screen los-home-final ${resumable?"has-resume":""}"><div class="los-home-plate" aria-hidden="true"></div><nav class="los-home-controls" aria-label="Game menu"><button id="homeHow" class="los-home-hotspot" data-home-hotspot="how" aria-label="How to play"><span class="sr-only">How to Play</span></button><button id="homeSettings" class="los-home-hotspot wc-settings-gear wc-settings-gear-home" data-home-hotspot="settings" data-west-coast-settings aria-label="Open Settings">${westCoastSettingsGearArt()}</button><button id="start" class="los-home-hotspot" data-home-hotspot="play" aria-label="Play"><span class="sr-only">Play</span></button>${resumable?`<button id="resumeSaved" class="los-home-resume" aria-label="Resume game — continue where you left off">RESUME GAME</button>`:""}</nav></section>${state.voiceOn?`<div id="voiceDiagnostic" class="voice-diagnostic">MIC LISTENING</div>`:""}`;
...
function packs(focusGroup="",focusValue=""){
 ...
 app.innerHTML=shell("CHOOSE YOUR GAME",`<div class="pack-picker"><div id="packError" class="setup-error" role="alert" aria-live="assertive"></div><section class="hierarchy-panel"><div class="pack-instruction">AUDIENCE / GAME ENVIRONMENT</div><div class="pack-grid audience-grid">${optionButtons(AUDIENCE_OPTIONS,audiences,"data-audience")}</div></section><section class="hierarchy-panel"><div class="pack-instruction">TOPICS / VIBES — MIX ANY</div><div class="pack-grid topic-grid">${optionButtons(TOPIC_OPTIONS,topics,"data-topic")}</div><div class="pack-summary">${topics.size?"SELECTED TOPICS ACTIVE":"NO TOPIC SELECTED — GENERAL MIX"}</div></section>${subcategoryScreen}</div>`,`<button id="back" class="btn">BACK</button><button id="continuePacks" class="btn primary large" data-voice="PLAY">PLAY</button>`);
...
function openPlayerEditor(existing=null,onComplete=null){
 const draft=normalizePlayer(existing||{}),overlay=document.createElement("div");overlay.className="los-overlay";overlay.setAttribute("role","dialog");overlay.setAttribute("aria-modal","true");overlay.setAttribute("aria-label",existing?"Edit player":"Add player");
 const draw=()=>{const avatar=avatarById(draft.avatar);overlay.innerHTML=`<form class="los-sheet los-player-sheet"><header><div><div class="los-sheet-kicker">WHO’S PLAYING?</div><h2>${existing?"EDIT PLAYER":"ADD PLAYER"}</h2></div><button type="button" class="los-close" data-close aria-label="Close player form">×</button></header><button type="button" class="los-player-avatar-edit" data-pick-avatar>${avatarArt(avatar)}<span>CHOOSE AVATAR</span></button><label>FIRST NAME<input name="firstName" autocomplete="given-name" maxlength="32" required value="${esc(draft.firstName)}"></label><label>LAST NAME <small>OPTIONAL</small><input name="lastName" autocomplete="family-name" maxlength="40" value="${esc(draft.lastName)}"></label><label>NICKNAME <small>OPTIONAL</small><input name="nickname" maxlength="32" value="${esc(draft.nickname)}"></label><div class="roster-error" role="alert"></div><button class="los-action los-action-primary" type="submit">SAVE PLAYER</button></form>`;
   overlay.querySelector("[data-close]").onclick=()=>overlay.remove();overlay.querySelector("[data-pick-avatar]").onclick=()=>openAvatarPicker(draft,()=>draw());overlay.querySelector("form").onsubmit=e=>{e.preventDefault();const form=new FormData(e.currentTarget);draft.firstName=String(form.get("firstName")||"").trim();draft.lastName=String(form.get("lastName")||"").trim();draft.nickname=String(form.get("nickname")||"").trim();draft.name=playerDisplayName(draft);if(!draft.firstName){overlay.querySelector(".roster-error").textContent="FIRST NAME IS REQUIRED";return}const saved=upsertProfile(draft),index=state.players.findIndex(p=>p.id===saved.id);if(index>=0)state.players[index]=saved;else if(state.mode!=="solo"||state.players.length===0)state.players.push(saved);state.selectedIds=state.players.map(p=>p.id);overlay.remove();players();onComplete?.(saved)}
...
function transition(kind,done,reason="game-transition"){
 ...
 if(kind==="lockIn"){hostSystem?.emit("lockIn",{mode:state.mode,name:p?.name||"player"});flowTimer=setTimeout(complete,1600)}else if(kind==="question"){hostSystem?.emit("lockIn",{mode:state.mode});flowTimer=setTimeout(complete,1600)}else flowTimer=setTimeout(complete,kind==="elimination"?2200:1800)
}
...
function question(resumeCurrent=false){
 ...
 app.innerHTML=`<section class="screen wc-master-screen wc-master-question"><div class="wc-master-stage">${westCoastGlobalControlsMarkup({showMic:false})}<header class="wc-question-player" aria-label="Current player: ${esc(playerName)}"><div class="wc-question-avatar">${avatarArt(activePlayer.avatar)}</div><div class="wc-question-player-copy"><strong class="wc-question-player-name ${playerNameSize}">${esc(playerName)}</strong></div></header><main class="content wc-master-board wc-question-board"><section class="question-area wc-question-live" aria-label="Question for ${esc(playerName)}"><div class="wc-question-presentation"><div class="question-text ${questionSize}">${esc(g.current.q)}</div></div>
