...
test("visible-button voice navigation executes safe exact commands early and destructive commands only on final",()=>{
 ...
 {const{h,state}=make();try{assert.equal(h.document.querySelector("#showtimeTitle").textContent.replace(/\s+/g,""),"IT’SSHOWTIME");assert.deepEqual([...h.document.querySelector("#showtimeTitle").children].map(e=>e.textContent),["IT’S","SHOWTIME"]);h.speak("start",{final:false,confidence:.2});assert.equal(state.screen,"handoff");h.speak("start",{final:true});assert.equal(state.screen,"handoff");const attempt=h.api.getVoiceDiagnostics().find(x=>x.stage==="visible-navigation-attempt"&&x.accepted);assert.equal(attempt.matchType,"exact-label");assert.equal(attempt.matchedTargetCount,1)}finally{h.close()}}
