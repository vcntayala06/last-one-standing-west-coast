...
test("Who’s In real setup lifecycle owns a listening recognizer and exact interim navigation advances once",()=>{
 for(const phrase of ["continue","next","start"]){const h=createHarness();try{const state=h.api.getState();h.click("#start");h.timers.advance(220);h.click("#continuePacks");h.timers.advance(220);assert.equal(state.screen,"players",phrase);state.players=[{id:"p1",name:"Alex"},{id:"p2",name:"Blair"}];h.api.players();const recognizer=h.recognition(),voice=h.window.__LOS_PLAYTEST_DIAGNOSTICS__.snapshot().recognition;assert.equal(voice.state,"listening",phrase);assert.equal(voice.owner.screen,"players",phrase);recognizer.emit(phrase,{final:false,confidence:.99,resultIndex:0});assert.equal(state.screen,"ready",phrase+" interim");recognizer.emit(phrase,{final:true,confidence:.99,resultIndex:0});assert.equal(state.screen,"ready",phrase);recognizer.emit(phrase,{final:true,confidence:.99,resultIndex:0});assert.equal(state.screen,"ready",phrase+" duplicate final");assert.equal(state.game,null)}finally{h.close()}}
});

test("Who’s In executes an exact interim command without waiting for WebKit finalization",()=>{
 for(const phrase of ["continue","next","start"]){const h=createHarness();try{const state=h.api.getState();state.mode="original";state.players=[{id:"p1",name:"Alex"},{id:"p2",name:"Blair"}];state.screen="players";h.api.players();const recognition=h.recognition();recognition.emit(phrase,{final:false,confidence:.8,resultIndex:0});assert.equal(state.screen,"ready",phrase);recognition.speechEnd();h.timers.advance(240);assert.equal(state.screen,"ready",phrase+" remains single action")}finally{h.close()}}
});
...
test("roster validation occurs on Who's In and valid Continue reaches Showtime",()=>{
 const h=createHarness();try{const state=h.api.getState();state.mode="original";state.players=[{id:"p1",name:""}];state.screen="setup";h.api.setup();h.click("#startGame");h.timers.advance(220);assert.equal(state.screen,"packs");h.click("#continuePacks");assert.equal(state.screen,"players");h.timers.advance(220);h.click("#continue");assert.equal(state.screen,"players");assert.match(h.document.getElementById("rosterError").textContent,/two named/i);state.players=[{id:"p1",name:"Alex"},{id:"p2",name:"Alex"}];h.api.players();h.click("#continue");assert.equal(state.screen,"players");assert.match(h.document.getElementById("rosterError").textContent,/unique/i);state.players[1].name="Blair";h.api.players();h.click("#continue");assert.equal(state.screen,"ready");const summary=h.document.querySelector(".wc-showtime-summary").textContent;assert.match(summary,/MULTIPLAYER.*MEDIUM.*15 SEC/);assert.doesNotMatch(summary,/VOICE|READ/)}finally{h.close()}
});
...
test("exact Back may execute on interim and moves exactly one setup screen",()=>{
 for(const [mode,screen,target] of [["original","players","packs"],["original","ready","players"],["solo","ready","packs"]]){const h=createHarness();try{const state=h.api.getState();state.mode=mode;state.screen=screen;h.api[screen]();h.speak("back",{final:false});assert.equal(state.screen,target);h.speak("back",{final:true});assert.equal(state.screen,target);h.timers.advance(1000);assert.equal(state.screen,target);assert.notEqual(state.screen,"home")}finally{h.close()}}
});
...
test("visible-button voice navigation executes safe exact commands early and destructive commands only on final",()=>{
 ...
 {const{h,state}=make();try{assert.equal(h.document.querySelector("#showtimeTitle").textContent,"IT’S SHOWTIME");assert.deepEqual([...h.document.querySelector("#showtimeTitle").children].map(e=>e.textContent),["IT’S SHOWTIME"]);h.speak("start",{final:false,confidence:.2});assert.equal(state.screen,"handoff");h.speak("start",{final:true});assert.equal(state.screen,"handoff");const attempt=h.api.getVoiceDiagnostics().find(x=>x.stage==="visible-navigation-attempt"&&x.accepted);assert.equal(attempt.matchType,"exact-label");assert.equal(attempt.matchedTargetCount,1)}finally{h.close()}}
...
test("visible-button voice navigation rejects ambiguous labels instead of clicking the first control",withHarness(h=>{
 const state=h.api.getState();state.screen="setup";h.api.setup();const buttons=[...h.document.querySelectorAll("button")];for(const button of buttons)button.style.display="none";let clicks=0;for(let i=0;i<2;i++){const button=h.document.createElement("button");button.textContent="CONTINUE";button.dataset.voice="CONTINUE";button.onclick=()=>clicks++;h.document.body.appendChild(button)}h.speak("continue",{final:true});assert.equal(clicks,0);const attempt=[...h.api.getVoiceDiagnostics()].reverse().find(x=>x.stage==="visible-navigation-attempt"&&x.spokenPhrase==="continue");assert.equal(attempt.accepted,false);assert.equal(attempt.rejectionReason,"ambiguous-visible-match");assert.equal(attempt.matchedTargetCount,2)
}));

test("central visible-button resolver covers the approved primary navigation labels once",()=>{
 for(const label of ["Play","Next","Continue","Start Game","Resume","Back","Done"]){const h=createHarness();try{const state=h.api.getState();state.screen="setup";h.api.setup();for(const button of h.document.querySelectorAll("button"))button.style.display="none";const button=h.document.createElement("button");button.className="primary";button.textContent=label;let clicks=0;button.onclick=()=>clicks++;h.document.body.appendChild(button);h.speak(label,{final:false,confidence:.15});assert.equal(clicks,1,label+" interim");h.speak(label,{final:true,confidence:.99});assert.equal(clicks,1,label+" duplicate final")}finally{h.close()}}
});
